...
The model achieved exceptional performance on the test set with \textbf{99.47\% accuracy} (373 out of 375 images correctly classified). Only two images were misclassified, both involving basophils incorrectly predicted as neutrophils.
...
\midrule
\textbf{Overall} & \textbf{0.9947} & \textbf{0.9947} & \textbf{0.9947} & \textbf{375} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:metrics} shows that three classes (eosinophil, lymphocyte, monocyte) achieved perfect 100\% scores across all metrics, while basophil and neutrophil showed minimal errors (97.33\% on one metric each).

\subsection{Confusion Matrix}
Figure~\ref{fig:confusion} presents the confusion matrix for the test set. The diagonal elements show the correct classifications, with only two off-diagonal entries representing basophils misclassified as neutrophils. This confusion is understandable as both cell types are granulocytes with morphologically similar granular appearance.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{outputs/figures/confusion_matrix_test.png}
\caption{Confusion matrix on test set showing near-perfect classification with only 2 errors out of 375 predictions.}
\label{fig:confusion}
\end{figure}
...
The two misclassified images were both basophils predicted as neutrophils. This error pattern can be attributed to:

\begin{itemize}
    \item \textbf{Morphological Similarity:} Both are granulocytes with granular cytoplasm
    \item \textbf{Staining Variation:} Differences in staining intensity may affect granule visibility
    \item \textbf{Sample Size:} Basophils had 73 test samples vs. 75 for other classes
\end{itemize}

Despite these minor errors, the model demonstrates robust discrimination capability across all cell types.
