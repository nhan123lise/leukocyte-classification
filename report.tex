...
\textbf{Phase 2 - Fine-tuning:} All layers were unfrozen and fine-tuned with a reduced learning rate (0.0001) and early stopping patience=8. This allowed the model to adapt deeper features specifically for leukocyte morphology.
...
The model achieved exceptional performance: \textbf{99.47\% validation accuracy} and \textbf{99.20\% test accuracy} (372 out of 375 images correctly classified). Only three images were misclassified.
...
\midrule
\textbf{Accuracy} & -- & -- & \textbf{0.9920} & \textbf{375} \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:confusion} presents the confusion matrix for the test set, demonstrating exceptional classification performance with \textbf{no significant confusion patterns}. The matrix shows 99.20\% accuracy with only 3 errors out of 375 predictions.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{outputs/figures/confusion_matrix_test.png}
\caption{Confusion matrix showing near-perfect classification (99.20\% accuracy) with no significant confusion patterns.}
\label{fig:confusion}
\end{figure}

\subsection{Training Dynamics}
Figure~\ref{fig:training} illustrates the training and validation loss curves for both training phases. Phase 1 shows rapid convergence with the frozen backbone, achieving 99.47\% validation accuracy. The close alignment between training and validation curves indicates no overfitting.
...
Only 3 out of 375 test images were misclassified (0.80\% error rate), with \textbf{no significant confusion patterns}:

\begin{itemize}
    \item \textbf{Robust Classification:} Model distinguishes all 5 cell types with near-perfect accuracy
    \item \textbf{No Systematic Errors:} The 3 errors show no pattern indicating class confusion
...
\begin{itemize}
    \item \textbf{99.20\% test accuracy} with only 3 errors out of 375 images
